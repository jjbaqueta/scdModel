...
<-	
	...
	!delegate(MyActionCode, none, none, none);
.
...
+!getQuotes(
	...
	getMyName(Me)
<-	
	.send(Partner, achieve, quoteFor(DelegatorActionCode, DelegatorRequest, PartnerActionCode));
	!getQuotes(DelegatorActionCode, DelegatorRequest, T);
.
+!getQuotes(_,_,[]).
...
<-		
	if (Delegator \== self) {
		...
		environments.action_computeASR(DelegatorActionCode, MyActionCode, none, ASR);
...
<-	
	if (MyDelegator == none) {
		.send(manager, tell, status(finished, chain(MyActionCode, prospect(Result, Satisfaction, Regret, Delegatees))));	
	} 
	else {
		!mergeImpressions(References, ReferenceMeasure)
		environments.action_addOfferTax(MyActionCode, Delegatees, return(promise(Promise), outcome(Outcome), failure(Position)));
		environments.action_computeASR(MyDelegatorActionCode, MyActionCode, ASR, CurrentASR);
